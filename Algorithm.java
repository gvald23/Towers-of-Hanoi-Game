...
public class Algorithm{
    ...
    public static void recurAlgorithm(int numDisks){
        if (numDisks%2 == 0){ //even number of disks
            ...
            //cannot use break command outside a loop, so if statement is needed.
            if (!col1.isEmpty() || !col2.isEmpty()){
                //col1-col2
                if (col2.isEmpty()){ //col1 to col2
                    col2.add(col1.get(col1.size()-1));
        ...
        //recursion element
        if (!col1.isEmpty() || !col2.isEmpty()){
            recurAlgorithm(numDisks);
        }
    }

    // printing in the algorithm would cause repetitive prints, so a separate method was needed
    public static void printRecurAlgorithm(){
        //printing final columns
        for (int i = col3.size()-1; i >= 0; i--){
            System.out.println("\t\t" + " " + col3.get(i));
        }
